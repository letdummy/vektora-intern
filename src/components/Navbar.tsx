...
function Navbar() {
    ...
    return (
        <nav className="bg-[#1A191C] relative">
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <div className={`${isNavOpen ? 'absolute left-0 top-full' : 'hidden'} items-center md:block justify-between w-full md:w-auto md:order-1`} id="navbar-cta">
                    <ul className="flex flex-col font-medium p-4 md:p-0 md:space-x-8 rtl:space-x-reverse md:flex-row bg-[#1A191C] md:bg-transparent">
                        {navItems.map((item) => (
                            <li key={item.name}>
                                <a
                                    href={item.href}
